...
with tab1:
    ...
    with col_in1:
        prev_file = st.file_uploader("📁 Προηγούμενο Scorecard (.xlsx) - Προαιρετικό", type=["xlsx"])
    with col_in2:
        curr_file = st.file_uploader("📁 Τρέχον Scorecard (new scorecard.xlsx) *", type=["xlsx"])

    if st.button("🚀 Υπολογισμός Scorecard", type="primary", use_container_width=True):
        if curr_file is None:
            st.error("⚠️ Παρακαλώ επιλέξτε το Τρέχον Scorecard!")
        else:
